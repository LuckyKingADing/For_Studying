...
## 第4章 多层感知机 - mlp.ipynb 问答整理

### 激活函数的作用和意义（通俗易懂版）

**问题：激活函数在深度学习中的作用是什么？**

**核心答案：激活函数是神经网络的"灵魂"，没有它网络就是一堆线性运算，有了它网络才能真正学习复杂的世界。**
...
**问题：y.backward(torch.ones_like(x), retain_graph=True) 各部分含义？**
...
10层sigmoid网络：
  第10层梯度 = 0.25
  第9层梯度 = 0.25 × 0.25 = 0.0625
  第8层梯度 = 0.25^3 = 0.0156
  ...
  第1层梯度 = 0.25^10 ≈ 0.000001
...
**问题：ReLU的"正数通过，负数变0"指的是权重还是梯度？**

**核心答案：ReLU操作的是激活值（输入值），既不是权重也不是梯度！**
...
ReLU直接处理的是：激活值（前向传播）
ReLU影响的是：梯度传播（反向传播）
ReLU不直接处理的是：权重（通过梯度更新）
```

---

## 第4章 多层感知机 - mlp-scratch.ipynb 问答整理
...
## 第4章 多层感知机 - mlp-concise.ipynb 问答整理
...
net = nn.Sequential(
    nn.Flatten(),        # 第0层：展平
    nn.Linear(784, 256), # 第1层：线性变换
    nn.ReLU(),           # 第2层：激活函数
    nn.Linear(256, 10)   # 第3层：输出层
)

数据流：
  输入 → Flatten → Linear → ReLU → Linear → 输出
